...
@jax.jit
def min_angle(x: jnp.ndarray) -> jnp.float32:
    dots = jnp.dot(x.T, x)
    # set diagonal to -1 so that we don't count self-dots (dot==1, angle==0)
    dots = jnp.where(jnp.eye(dots.shape[0], dtype=bool), -1, dots)
  
    max_dot = jnp.clip(jnp.max(dots), -1, 1)  # make sure dot product is between -1 and 1
    min_angle = jnp.arccos(max_dot)  # JAX arrays are implicitly scalar when used in a scalar context
    return min_angle

@jax.jit
def calculate_gradient(x: jnp.ndarray, epsilon=1e-6) -> jnp.ndarray:
    d, n = x.shape
    gradient = jnp.zeros_like(x)
    
    def body_fun(ij, grad):
        i, j = ij // n, ij % n
        x_plus = x.at[i, j].add(epsilon)
        x_minus = x.at[i, j].add(-epsilon)
        
        # Normalize the perturbed vectors
        x_plus = x_plus / jnp.linalg.norm(x_plus, axis=0)
        x_minus = x_minus / jnp.linalg.norm(x_minus, axis=0)
        
        grad = grad.at[i, j].set((min_angle(x_plus) - min_angle(x_minus)) / (2 * epsilon))
        return grad

    gradient = jax.lax.fori_loop(0, d*n, body_fun, gradient)
...
def pso(f, d, n, population_size=1000, max_iterations=100, w=0.95, c1=0.5, c2=0.1, c3=0.5, verbose=False, plots=True, thread=0): #workhorse
    ...
    for i in range(max_adam_iterations):
        ...
        gradient = calculate_gradient(global_best)
